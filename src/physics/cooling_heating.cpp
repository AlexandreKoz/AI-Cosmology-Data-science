...
[[nodiscard]] double uvHeatingScale(const std::string& uv_background_model, double redshift) {
  if (uv_background_model == "none") {
    return 0.0;
  }
  if (uv_background_model == "hm12") {
    const double z_peak = 2.0;
    const double width = 2.5;
    const double dz = (redshift - z_peak) / width;
    return std::exp(-dz * dz);
  }
  if (uv_background_model == "fg20") {
    return 1.0 / (1.0 + std::pow((1.0 + redshift) / 4.0, 2.0));
  }
  throw std::invalid_argument("CoolingRateProvider: unsupported uv_background_model='" + uv_background_model + "'");
}

[[nodiscard]] double shieldingFactor(
    const std::string& self_shielding_model,
    double hydrogen_number_density_cgs) {
  if (self_shielding_model == "none") {
    return 1.0;
  }
  if (self_shielding_model == "rahmati13_like") {
    const double threshold = 1.0e-2;
    return 1.0 / (1.0 + std::pow(hydrogen_number_density_cgs / threshold, 1.6));
  }
  throw std::invalid_argument(
      "CoolingRateProvider: unsupported self_shielding_model='" + self_shielding_model + "'");
}
