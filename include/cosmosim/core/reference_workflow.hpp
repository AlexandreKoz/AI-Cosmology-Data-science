...
class ReferenceWorkflowRunner {
 public:
  explicit ReferenceWorkflowRunner(FrozenConfig frozen_config) : m_frozen_config(std::move(frozen_config)) {}

  [[nodiscard]] const FrozenConfig& frozenConfig() const noexcept { return m_frozen_config; }

  [[nodiscard]] ReferenceWorkflowReport run(
      const std::filesystem::path& run_directory,
      const ReferenceWorkflowOptions& options = {}) const {
    class StageAuditCallback final : public IntegrationCallback {
     public:
      ...
      void onStage(StepContext& context) override {
        m_stage_sequence->push_back(std::string(integrationStageName(context.stage)));
      }
    ...
    const SimulationConfig& config = m_frozen_config.config;
    ...
    SimulationConfig generated_ic_config = config;
    ...
    SimulationState state = std::move(ic_result.state);
    ...
    const ModePolicy mode_policy = buildModePolicy(config.mode);
    ...
    const LambdaCdmBackground background(background_config);
    ...
    ActiveSetDescriptor active_set{.particle_indices = particle_indices,
    ...
    std::vector<IntegrationStage> observed_stages;
    observed_stages.reserve(report.stage_sequence.size());
    for (const std::string& stage_name : report.stage_sequence) {
      for (const IntegrationStage expected_stage : StageScheduler::kickDriftKickOrder()) {
        if (stage_name == integrationStageName(expected_stage)) {
          observed_stages.push_back(expected_stage);
          break;
        }
      }
    }
    report.canonical_stage_order = isCanonicalIntegrationStageOrder(observed_stages);
    ...
    writeProfilerReportJson(profiler, report.profiler_json_path);
    writeProfilerReportCsv(profiler, report.profiler_csv_path);
