...
void testPeriodicSinusoidalForceResponse() {
  ...
  for (std::size_t i = 0; i < particle_count; ++i) {
    ...
    assert(std::abs(accel_y[i]) < 5.0e-2);
    assert(std::abs(accel_z[i]) < 5.0e-2);
  }
  ...
  assert(norm_got > 0.0);
#endif
  assert(profile.assign_ms >= 0.0);
  assert(profile.fft_forward_ms >= 0.0);
  assert(profile.fft_inverse_ms >= 0.0);
