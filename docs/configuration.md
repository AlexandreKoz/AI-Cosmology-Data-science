...
Baseline Schmidt-Kennicutt-inspired controls are normalized under `physics`:
- `enable_star_formation` (bool).
- `star_formation_epsilon_ff` in `[0, 1]` for `\dot{rho_*} = epsilon_ff * rho_gas / t_ff`.
- `star_formation_density_threshold_code` (> 0).
- `star_formation_temperature_threshold_k` (> 0).
- `star_formation_max_velocity_divergence_code` (typically `<= 0` to prefer converging flows).
- `star_formation_gravitational_constant_code` (> 0) used by `t_ff = sqrt(3 pi / (32 G rho_gas))`.
- `star_formation_minimum_particle_mass_code` (> 0).
- `star_formation_spawn_mode` (`deterministic` or `stochastic`).
- `star_formation_rng_seed` (uint64 provenance seed for rank-local reproducible draws).
