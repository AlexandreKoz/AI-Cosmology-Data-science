...
int main() {
  ...
  const double assign_throughput_mpart_s = static_cast<double>(particle_count) / (profile.assign_ms * 1.0e3);
  const double interp_throughput_mpart_s = static_cast<double>(particle_count) / (profile.interpolate_ms * 1.0e3);
  const double bytes_per_s = static_cast<double>(profile.bytes_moved) / (steady_ms * 1.0e-3);
  ...
  std::cout << "bench_pm_solver"
            ...
            << " setup_ms=" << setup_ms
            << " steady_ms=" << steady_ms
            << " assign_ms=" << profile.assign_ms
            << " fft_forward_ms=" << profile.fft_forward_ms
            << " poisson_ms=" << profile.poisson_ms
